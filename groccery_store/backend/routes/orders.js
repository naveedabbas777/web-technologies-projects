...
router.get('/admin/stats/all', verifyToken, isAdminOrStaff, cacheGet({ keyPrefix: 'cache:orders:stats:', ttlSeconds: 60 }), orderController.getOrderStats);
router.get('/rider/available', verifyToken, isDeliveryRider, cacheGet({ keyPrefix: 'cache:orders:rider-available:', ttlSeconds: 30 }), orderController.getRiderAvailableOrders);
router.get('/rider/my-orders', verifyToken, isDeliveryRider, cacheGet({ keyPrefix: 'cache:orders:rider-my:', ttlSeconds: 30, perUser: true }), orderController.getRiderOrders);
