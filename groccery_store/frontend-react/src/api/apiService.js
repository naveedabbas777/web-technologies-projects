...
class ApiService {
  ...
  async request(method, endpoint, data) {
    ...
    const notifyUnauthorized = (payload) => {
      if (!this.token || !this.unauthorizedHandler) {
        return;
      }

      const code = payload?.code || payload?.errorCode;
      const message = String(payload?.message || '').toLowerCase();
      const isAuthTokenError = code === 'AUTH_INVALID_TOKEN'
        || code === 'AUTH_TOKEN_EXPIRED'
        || message.includes('invalid token')
        || message.includes('token expired');

      if (response.status === 401 && isAuthTokenError) {
        this.unauthorizedHandler(payload);
      }
    };

    if (response.status === 204 || response.status === 304 || !trimmed) {
      return {
        status: 'success',
    ...
    const payload = {
      ...
      statusCode: response.status,
      message: trimmed || response.statusText || 'Request failed',
      data: null
    };
