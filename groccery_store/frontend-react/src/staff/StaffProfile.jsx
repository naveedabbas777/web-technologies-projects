...
export default function StaffProfile() {
  ...
  useEffect(() => {
    apiService.get('/auth/profile').then((resp) => {
      if (resp.status === 'success') {
        const parts = (resp.user?.name || '').split(' ');
        setProfile({
          firstName: parts[0] || '',
        ...
        setMessage(resp.message || 'Failed to load profile.');
      }
    }).catch(() => {
  ...
  return (
    <div className="admin-container">
      ...
      <div className="admin-content">
        <AdminTopbar title="Staff Profile" iconClass="fas fa-user-circle" />
        <div className="admin-page-content">
          {message && <div className="alert alert-warning">{message}</div>}
          {loading && <div className="alert alert-secondary">Loading profile...</div>}
          <div className="profile-card">
            <div className="profile-header">
              <div className="profile-avatar-container-admin">
                ...
                <label htmlFor="staffAvatarUpload" className="avatar-upload-btn-admin" title="Click to change profile picture">
                  <i className="fas fa-camera"></i>
                </label>
                <input
                  type="file"
                  id="staffAvatarUpload"
                  accept="image/*"
