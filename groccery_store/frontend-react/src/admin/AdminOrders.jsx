...
export default function AdminOrders({ SidebarComponent = AdminSidebar }) {
  ...
  const STATUS_META = [
    ...
    { key: 'processing', label: 'Processing', color: '#f97316' },
    { key: 'shipped', label: 'Shipped', color: '#ec4899' },
  ...
  const statusSummary = useMemo(() => {
    const counts = STATUS_META.reduce((acc, status) => {
      acc[status.key] = 0;
      return acc;
    }, {});
  ...
  useEffect(() => {
    ...
    const data = STATUS_META.map((status) => statusSummary[status.key] || 0);

    chartRef.current = new Chart(canvasRef.current, {
      type: 'doughnut',
      data: {
        labels: STATUS_META.map((status) => status.label),
        datasets: [
          {
            data,
            backgroundColor: STATUS_META.map((status) => status.color),
            borderWidth: 0
          }
        ]
      },
  ...
  }, [JSON.stringify(statusSummary), STATUS_META]);
                ...
                <div className="orders-status-list">
                  {STATUS_META.map((status) => (
                    <div className="status-row" key={status.key}>
                      ...
                      <strong>{statusSummary[status.key] || 0}</strong>
                    </div>
                  ))}
                </div>
              </div>
