...
function Login() {
  ...
  const login = async () => {
    ...
    try {
      // Check which sign-in methods are registered for this email.
      // This helps show a helpful message if the account was created with Google/GitHub etc.
      ...
      // Map a couple of common auth errors to friendly messages
  ...
  return (
    <section style={{ padding: '80px 10%' }}>
      <div className="glass" style={{ maxWidth: 420, margin: '0 auto' }}>
        ...
        {error && (
          <div className={`status ${error && error.toLowerCase().includes('password reset') ? 'success' : 'error'}`}>
            {error}
          </div>
        )}
