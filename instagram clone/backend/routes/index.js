...
router.get('/', async (req, res, next) => {
  ...
  const feedType = req.query.feed || 'following';
  ...
  try {
    ...
    // If user is logged in and wants to see their following feed
    if (req.session.userId && feedType === 'following') {
    ...
    for (let i = 0; i < posts.length; i++) {
      posts[i].commentCount = await Comment.countDocuments({ imagePost: posts[i]._id });
    }
    ...
    if (req.xhr || req.headers.accept.indexOf('json') > -1) {
      ...
      res.render('home', {
        ...
        feedType: feedType,
        user: req.session.userId ? { _id: req.session.userId, role: req.session.role } : null
      });
    }
  } catch (err) {
    console.error(err);
