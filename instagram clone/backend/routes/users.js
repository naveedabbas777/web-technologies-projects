...
router.get('/:userId/profile', async (req, res) => {
  try {
    const appLogger = require('../utils/logger');
    appLogger.debug('Accessing /users/:userId/profile for userId: %s', req.params.userId);
...
router.get('/:userId/followers', async (req, res) => {
  try {
    const user = await User.findById(req.params.userId).populate('followers', '_id username profilePicture');
    if (!user) {
...
// GET /users/:userId/following - Get a user's following list
router.get('/:userId/following', async (req, res) => {
  try {
    const user = await User.findById(req.params.userId).populate('following', '_id username profilePicture');
    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }
